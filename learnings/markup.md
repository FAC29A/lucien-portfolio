...
- `<H1>` `<H2>`  elements are used in the correct order to ensure that they would be read out by a screen reader. (see below image)
...
- We used alt-tags on each image (for the empty image, we use `alt=""` so the machine understands this is intentionally left blank description). This is important for those with sight access requirements to be able to use screen readers to acquire the description of images.


## 2. Ensure a web page is readable for screen readers

We tested the site via various screen readers (Chrome, Safari, Mac screen reader). The important content and sections are successfully read out on these screen readers.


## 3. Ensure our UI has sufficient colour contrast so that everyone can perceive it comfortably

We use the color contrast checker suggested by Lighthouse (https://dequeuniversity.com/rules/axe/4.7/color-contrast) to ensure they pass WCAG Standard AAA.
...
We use various tools to verify our accessibility rate:

- https://www.a11yproject.com/checklist/ (see below image)
- https://validator.w3.org/nu/
- Lighthouse on Google Chrome: Our site rates 94% on accessibility score in Lighthouse. (see below image)
...
The site has media queries to adjust for small screens (max-screen-width 600px) and medium screens (max-screen-width 900px).
...
The site is responsive for mobile users (see below image). However, for this project, we did not start building using a mobile-first approach. We started to build the site from a wide-screen user. This is a note to take for the next project.
...
In CSS, we use color variables to apply all the elements on the site (see below image).
...
We use CSS Grid to style <article> with a two-direction layout.
...
In our Git commit - we can see in which section was changed, and what was the purpose of the change.
